...
<LoginGuard>
	<Tabs bind:active>
		{#snippet tabs()}
			<a href="{base}/boards/{record.id}/">
				<Tab key="view" pathname="/boards/{record.id}/">View</Tab>
			</a>
			{#if $authModel?.id === record.user || client.authStore.isAdmin}
				<a href="{base}/boards/{record.id}/edit/">
					<Tab key="edit" pathname="/boards/{record.id}/edit/">Edit</Tab>
				</a>
				<a href="{base}/boards/{record.id}/delete/">
					<Tab key="delete" pathname="/boards/{record.id}/delete/">Delete</Tab>
				</a>
			{/if}
			<a href="{base}/boards/{record.id}/auditlog/">
				<Tab key="auditlog" pathname="{base}/boards/{record.id}/auditlog/">
					Audit Log
				</Tab>
			</a>
		{/snippet}
		<!-- tab content -->
		{#if active === "auditlog"}
			{#await preloadData(`${base}/auditlog/boards/${record.id}/`) then result}
